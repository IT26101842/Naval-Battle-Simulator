...
int main() {
    ...
    printf("========================================\n");
    printf("   NAVAL BATTLE SIMULATOR - PART 2C     \n");
    printf("========================================\n");
    printf("1. Static Battlefield (Part 1-A + Degradation)\n");
    printf("2. Moving Battleship (Part 1-B Sim 1 + Degradation)\n");
    printf("3. Jammed Gun Mode   (Part 1-B Sim 2 + Degradation)\n");
    ...
    // 📉 Impact Power Degradation (Gamma - y) Configuration
    double gamma_b;
    double gamma_e[4];
    printf("\n--- Impact Power Degradation (Gamma) Configuration ---\n");
    printf("1. Enter manually (Guided input)\n");
    printf("2. Generate randomly within a suggested range\n");
    printf("Select option (1-2): ");
    int gamma_option;
    if (scanf("%d", &gamma_option) != 1) gamma_option = 1;

    if (gamma_option == 1) {
        printf("\n[Manual Mode] Enter Gamma values (close to zero for B, relatively higher for E):\n");
        printf("Enter Gamma for Battleship [Suggested: 0.001 to 0.020]: ");
        scanf("%lf", &gamma_b);
        printf("Enter Gamma for Escort EA  [Suggested: 0.020 to 0.050]: ");
        scanf("%lf", &gamma_e[0]);
        printf("Enter Gamma for Escort EB  [Suggested: 0.030 to 0.060]: ");
        scanf("%lf", &gamma_e[1]);
        printf("Enter Gamma for Escort EC  [Suggested: 0.025 to 0.055]: ");
        scanf("%lf", &gamma_e[2]);
        printf("Enter Gamma for Escort ED  [Suggested: 0.040 to 0.080]: ");
        scanf("%lf", &gamma_e[3]);
    } else {
        printf("\n[Random Mode] Enter ranges (Use small decimal values to avoid instant power drop):\n");
        double b_min, b_max;
        printf("Battleship Gamma Min & Max (e.g., 0.005 0.015): ");
        scanf("%lf %lf", &b_min, &b_max);
        if (b_max < b_min) { double tmp = b_min; b_min = b_max; b_max = tmp; }
        gamma_b = b_min + ((double)rand() / RAND_MAX) * (b_max - b_min);

        double e_min, e_max;
        printf("Escort Ships Gamma Min & Max (e.g., 0.030 0.070): ");
        scanf("%lf %lf", &e_min, &e_max);
        if (e_max < e_min) { double tmp = e_min; e_min = e_max; e_max = tmp; }
        for (int i = 0; i < 4; i++) {
            gamma_e[i] = e_min + ((double)rand() / RAND_MAX) * (e_max - e_min);
        }
    }
    printf("Configured Gamma -> B: %.4f | EA: %.4f | EB: %.4f | EC: %.4f | ED: %.4f\n",
           gamma_b, gamma_e[0], gamma_e[1], gamma_e[2], gamma_e[3]);
    ...
    if (sim_choice == 1) {
        run_part2c_static_simulation(b, escorts, n_escorts, tb_q, te_p, gamma_b, gamma_e);
    } else {
        int steps;
        ...
        if (sim_choice == 2) {
            run_part2c_moving_simulation(b, escorts, n_escorts, path, steps, tb_q, te_p, gamma_b, gamma_e);
            ...
            run_part2c_jammed_simulation(b, escorts, n_escorts, path, steps, jam_step, jam_min_angle, tb_q, te_p, gamma_b, gamma_e);
        }
    }
